...
const NewPiggyBankPage = () => {
  ...
  return (
    <Layout>
        ...
        */}
        ...
        {piggyBankType === "amount" ? (
          ...
          <div>
            ...
            <br />
            <div className="shadow appearance-none border rounded font-bold text-gray-500 py-4 px-6 leading-tight focus:outline-none focus:shadow-outline">
              <DatePicker className="w-80 text-xl"
                selected={startDate}
     ...
     </div> :        <button disabled = {loader} className="rounded border bg-pink-100 border-pink-300 py-1 px-4 text-xl hover:bg-pink-300"type="submit">
       Create
     </button>)
              : null}
...
{!currentAccount ?  <div className="flex justify-center"><button
      ...
      {error ? <div className={error=="Transaction success" ? "аlex justify-center w-1/3 text-green-800 border bg-green-400 py-1 px-4 font-bold text-3xl" : "flex justify-center w-1/3 text-red-800 border bg-red-400 py-1 px-4 font-bold text-3xl"}>{error}</div> : null}
      </div>
    </Layout>
  );
};
